...
function enhanceRegistrationForm(){
  ...
  year.innerHTML=['Select your batch','2025','2026','2027','2028','2029'].map((x,i)=>`<option value="${i?x:''}">${x}</option>`).join('');
  ...
  course.innerHTML=['Select your course','B.Tech','BCA','BBA','B.Des','Other'].map((x,i)=>`<option value="${i?x:''}">${x}</option>`).join('');
